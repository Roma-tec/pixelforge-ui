...
| **Сайт из макетов** | **`site/`** — 18 страниц реального сайта, 83 ссылки, светлая и тёмная темы |
...
Папка **`site/`** — это готовый статический сайт, собранный из макетов проекта.
Каждый экран стал отдельной страницей с настоящими ссылками: переходы работают
как в обычном сайте, кнопка «назад» в браузере тоже работает.

**Открыть:** `site/index.html` (страница «Карта экранов») → оттуда можно перейти на любой экран.

**Страницы:**

| Файл | Что это |
|---|---|
| `index.html` | карта сайта — все экраны плиткой, кликабельны |
| `login.html` | S1 Вход / регистрация |
| `dashboard.html` | S0 Мои проекты |
| `editor.html` | S2 Редактор — холст |
| `layers.html` | S4 Слои |
| `filters.html` | S5 Фильтры и эффекты |
| `history.html` | S7 История действий |
| `brushes.html` | S8 Кисти и палитра |
| `export.html` | S6 Экспорт |
| `templates.html` | S10 Библиотека шаблонов |
| `settings.html` | S3 Настройки профиля |
| `admin.html` | S11 Администрирование |
| `mobile.html` / `mobile-editor.html` | S9 мобильная версия (портрет / ландшафт) |
| `ui-kit.html`, `moodboard.html` | дизайн-система |
| `flow-map.html`, `use-case.html` | диаграммы |

**Возможности:**
- 83 настоящие ссылки — навигация как в готовом продукте;
- каждая кликабельная зона подсвечивается при наведении и показывает подсказку сверху;
- кнопка **☀ / ☾** (или клавиша `T`) — светлая / тёмная тема, выбор сохраняется между страницами;
- кнопка **◎** (или `H`) — подсветить все кликабельные зоны (удобно на защите).

> При публикации через GitHub Pages сайт открывается по адресу
> `https://<логин>.github.io/pixelforge-ui/site/`
...
1. **Сайт:** откройте `site/index.html` в браузере — карта экранов, оттуда переходы на любую страницу.
...
```
pixelforge-ui/
├─ site/                    сайт из макетов (18 страниц, навигация ссылками)
│  └─ index.html             карта сайта — все экраны
